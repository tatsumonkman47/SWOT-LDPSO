...
import inox.random           # For RNG context management
...
import zarr                  # For zarr dataset handling
import signal                # For timeout handling
import threading             # For prefetch functionality
import queue                 # For prefetch functionality
...
try:
    ...
    PATH = Path.cwd()  # Use current working directory as default
    print(f"[SETUP] PATH not defined, using current directory: {PATH}")
...
def log_memory_usage(stage=""):
    ...
    try:
        ...
        print(f"[GPU {stage}] GPUtil not available, skipping GPU memory logging")
    except Exception as e:
        print(f"[GPU {stage}] Error getting GPU memory: {e}")
...
CONFIG = {
    ...
    # Training settings
    'epochs': 256,
    'batch_size': 384,  # Increased from 256 for better GPU utilization
    'scheduler': 'constant',
    'lr_init': 2e-4,
    'lr_end': 1e-6,
    'lr_warmup': 0.0,
...
def generate(model, dataset, rng, batch_size, chunk_size=None, **kwargs):
    ...
    # Return a ChunkedDataset that can handle the chunked data efficiently
...
def train(runid: int, lap: int, src: str):
    ...
    seed = hash((f"runs/training_{runid}", lap)) % 2**16
    rng = inox.random.PRNG(seed)
    print(f"[TRAIN] Using seed: {seed}")
    
    # Set up global inox RNG context for the entire training process
    init_rng = inox.random.PRNG(rng.split())
    dropout_rng = inox.random.PRNG(rng.split())
    
    print(f"[TRAIN] Setting up RNG context with init_rng and dropout_rng")
    print(f"[TRAIN] Passing seed={seed} to inner function")
    
    with inox.random.set_rng(init=init_rng, dropout=dropout_rng):
        print(f"[TRAIN] RNG context set up successfully")
        print(f"[TRAIN] Available RNG states: {list(inox.random.INOX_RNG.keys())}")
        return _train_inner(runid, lap, src, rng, seed)

def _train_inner(runid: int, lap: int, src: str, rng: inox.random.PRNG, seed: int):
    """Inner training function that runs within the RNG context"""
    
    print(f"[TRAIN] _train_inner called successfully with seed={seed}")
    print(f"[TRAIN] Function signature: _train_inner(runid={runid}, lap={lap}, src='{src}', rng=<PRNG>, seed={seed})")
    ...
    # Use consistent runpath based on runid only, not wandb run name
    runpath = PATH / f'runs/{runid}'
    ...
    if lap > 0:
        ...
        if not checkpoint_path.exists():
            print(f"[TRAIN] ERROR: Checkpoint {checkpoint_path} does not exist!")
            print(f"[TRAIN] Available files in {runpath}:")
            if runpath.exists():
                for file in runpath.iterdir():
                    print(f"[TRAIN]   - {file.name}")
            else:
                print(f"[TRAIN]   - Directory {runpath} does not exist!")
            
            # Wait and retry a few times in case the previous job is still running
            max_retries = 30  # Wait up to 5 minutes
            for retry in range(max_retries):
                print(f"[TRAIN] Waiting for checkpoint... (attempt {retry + 1}/{max_retries})")
                time.sleep(10)  # Wait 10 seconds
                if checkpoint_path.exists():
                    print(f"[TRAIN] Checkpoint found after {retry + 1} retries!")
                    break
            else:
                raise FileNotFoundError(f"Checkpoint {checkpoint_path} not found after waiting {max_retries * 10} seconds")
        
        previous = load_module(checkpoint_path)
        print(f"[TRAIN] Successfully loaded checkpoint from {checkpoint_path}")
    else:
    ...
    if lap > 0:
        ...
        print(f"[TRAIN] Current RNG context: {list(inox.random.INOX_RNG.keys())}")
        model = make_model(key=rng.split(), in_channels=C, out_channels=C, **CONFIG)
        print(f"[TRAIN] Model created successfully") 
    ...
    for epoch in (bar := trange(config.epochs, ncols=88)):
        ...
        if (epoch + 1) % 16 == 0:
            ...
            num = x.shape[0]
            cols = int(math.sqrt(num))  # Use math.sqrt instead of np.sqrt for consistency
            rows = num // cols
...
if __name__ == '__main__':
    ...
    for lap in range(32):
        ...
        if len(jobs) > 1:
            # Wait for successful completion, not "any" status
