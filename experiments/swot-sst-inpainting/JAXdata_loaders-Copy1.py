...
class JAXLLC4320Dataset:
    ...
    def _load_patch(self, idx=None, patch_id=None):
        if patch_id is None:
            ...
            coords = None
        invars, masks = self._load_patch_fields(patch_id, self.infields, self.in_transform_list, self.in_mask_list)
        invar = np.stack(invars, axis=-1) # Stacking s.t. shape is (B,T,H,W,C)
        mask = np.stack(masks, axis=-1) # Stacking s.t. shape is (B,H,W,C)
        if self.flatten:
    ...
    def get_cloud_mask_rho(self):
        ...
        k_np_rot = np.random.randint(4) # Add a random rotation (for augmenting synthetic data)
